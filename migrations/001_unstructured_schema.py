"""
Migration 001: Unstructured Data Schema

Date: 2024-01-15
Applied: Yes (historical migration)

Description:
    Adds unstructured data extraction capabilities to the FinLoom database:
    - Enhances sections table with metadata columns
    - Enhances tables table with extraction metadata
    - Creates footnotes table for footnote extraction
    - Creates chunks table for semantic chunking (RAG)

Changes:
    - sections: +11 columns (section_part, subsections, extraction_quality, etc.)
    - tables: +8 columns (table_markdown, is_financial_statement, etc.)
    - NEW TABLE: footnotes (footnote extraction and linking)
    - NEW TABLE: chunks (hierarchical semantic chunking)
    - NEW INDEXES: Performance optimization for queries

Dependencies:
    - Requires existing sections and tables tables
    - Requires filings table for foreign keys

Notes:
    - This migration is idempotent (safe to run multiple times)
    - Foreign keys ensure referential integrity
    - JSON columns store nested metadata structures
"""
...
logger = get_logger("finloom.migration.001")

# Migration metadata
MIGRATION_NUMBER = 1
MIGRATION_NAME = "unstructured_schema"
...
def create_chunks_table(conn: duckdb.DuckDBPyConnection) -> None:
    """Create chunks table for semantic chunking (RAG)."""
...
def run_migration(db_path: Path) -> bool:
    ...
    logger.info(f"Running migration {MIGRATION_NUMBER:03d}_{MIGRATION_NAME}")
    ...
    try:
        ...
        if success:
            logger.info(f"✅ Migration {MIGRATION_NUMBER:03d}_{MIGRATION_NAME} completed successfully")
        else:
            logger.error(f"❌ Migration {MIGRATION_NUMBER:03d}_{MIGRATION_NAME} verification failed")
...
def main():
    ...
    print("\n" + "="*80)
    print(f"  DATABASE MIGRATION: {MIGRATION_NUMBER:03d}_{MIGRATION_NAME}")
    print("="*80)
    print(f"\nMigration Date: {MIGRATION_DATE}")
    print(f"Description: Unstructured data schema")
