...
-- Sections: Extracted text sections from filings
CREATE TABLE IF NOT EXISTS sections (
    id INTEGER PRIMARY KEY,
    accession_number VARCHAR NOT NULL,
    
    -- Section identification
    section_type VARCHAR NOT NULL,          -- item_1, item_1a, item_7, etc.
    section_title VARCHAR,                  -- Extracted section title
    section_number VARCHAR,                 -- Original section number (1, 1A, 7, etc.)
...
-- Tables: Extracted tables from filings
CREATE TABLE IF NOT EXISTS tables (
    ...
    table_name VARCHAR,
    table_type VARCHAR,                     -- financial, narrative, schedule, other
    table_index INTEGER,                    -- Order in document
    
    -- Structure
    table_data JSON,                        -- Full table as JSON
    headers JSON,                           -- Column headers
    row_count INTEGER,
    column_count INTEGER,
    
    -- Source location
    source_element VARCHAR,                 -- HTML element ID or path
    
    created_at TIMESTAMP 
);

-- Indexes for table queries
CREATE INDEX IF NOT EXISTS idx_tables_accession ON tables(accession_number);
