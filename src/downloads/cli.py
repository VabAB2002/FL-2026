...
Usage:
    python -m src.downloads                    # Full backfill
    python -m src.downloads --resume           # Resume from checkpoint
    python -m src.downloads --company CIK      # Single company
    python -m src.downloads --download-only    # Download only
    python -m src.downloads --parse-only       # Parse only
"""
...
from src.downloads.downloader import SECDownloader
from src.downloads.sec_api import SECApi
from src.readers.xbrl_reader import SimpleXBRLParser, XBRLParser
...
from src.infrastructure.config import get_settings, load_config
from src.infrastructure.logger import get_logger, setup_logging
from src.checks.data_quality import DataQualityChecker

logger = get_logger("finloom.downloads.cli")
...
def main():
    ...
    if total_stats.get("download"):
        ...
        logger.info(
            f"Downloaded: {d.get('filings_downloaded', 0)} filings, "
