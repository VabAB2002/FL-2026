...
from ..infrastructure.config import get_absolute_path, get_env_settings, get_settings
from ..infrastructure.logger import get_logger
from ..infrastructure.request_throttle import get_rate_limiter
...
class SECDownloader:
    ...
    def _resolve_cik_dir(self, year_dir: Path, cik_padded: str) -> Path:
        """
        Find the CIK directory, handling both old (bare CIK) and new
        (CIK_TICKER_Company) naming conventions.

        Args:
            year_dir: The year directory to search in.
            cik_padded: Zero-padded CIK string.

        Returns:
            Path to the matching directory (existing or default).
        """
        # Check for renamed folder first (e.g., 0000320193_AAPL_Apple)
        if year_dir.exists():
            for d in year_dir.iterdir():
                if d.is_dir() and d.name.startswith(cik_padded):
                    return d
        # Fall back to bare CIK
        return year_dir / cik_padded
    ...
    def get_filing_path(
        ...
        Path structure: {output_dir}/{year}/{cik_ticker_company}/{accession}/

        Args:
            cik: Company CIK.
            accession_number: Filing accession number.
            ticker: Company ticker (e.g., "AAPL"). Used for folder naming.
            company_name: Company name (e.g., "Apple Inc"). Used for folder naming.
        ...
        # Try to find an existing directory for this CIK (handles renames)
        cik_dir = self._resolve_cik_dir(year_dir, cik_padded)

        # If creating a new directory and ticker info is available, use named format
        if not cik_dir.exists() and ticker:
            safe_name = (company_name or "").replace(" ", "_").replace(".", "").replace(",", "")
            cik_dir = year_dir / f"{cik_padded}_{ticker}_{safe_name}"

        return cik_dir / acc_clean
    ...
    def download_filing(
        ...
        Args:
            ...
            ticker: Company ticker for folder naming.
        ...
        filing_path = self.get_filing_path(
    ...
    def download_company_filings(
        ...
        for i, filing in enumerate(filings):
            ...
            result = self.download_filing(filing, ticker=ticker, company_name=company_name)
